        ...
        {posts.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {posts.map((post) => {
              ...
              return (
                <Link key={post.id} href={`/ko/notice/${post.id}/`} style={{ display: 'block', textDecoration: 'none', color: 'inherit', background: '#0a1628', border: '1px solid rgba(31,74,117,.5)', padding: '20px 24px', transition: 'border-color .2s', position: 'relative' }}>
                  {isPinned && (
                    <span style={{ position: 'absolute', top: 16, right: 16, fontFamily: 'monospace', fontSize: 8, padding: '2px 6px', background: 'rgba(45,212,191,.1)', border: '1px solid rgba(45,212,191,.3)', color: '#2dd4bf' }}>중요</span>
                  )}
