...
function renderBlock(block: any) {
  ...
  switch (type) {
    ...
    case 'image': {
      ...
      return url ? <div key={id} style={{ margin: '20px 0', borderRadius: 4, overflow: 'hidden' }}><img src={url} alt="" style={{ width: '100%', display: 'block' }} /></div> : null;
    }
    case 'code':
...
export default function NoticePage() {
  ...
  useEffect(() => {
    fetchNotionDB(process.env.NEXT_PUBLIC_NOTION_NOTICE_DB_ID!)
      .then(setPosts)
      .catch(console.error);
  }, []);
  ...
  const openPost = async (post: any) => {
    ...
    try {
      const b = await fetchNotionBlocks(post.id);
      setBlocks(b);
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#050d1a', color: '#e8f1ff', paddingTop: 96, paddingBottom: 80, paddingLeft: 'clamp(16px,4vw,48px)', paddingRight: 'clamp(16px,4vw,48px)', fontFamily: "'Pretendard', sans-serif" }}>
            ...
            {posts.map((post) => {
              ...
              return (
                <div key={post.id} onClick={() => openPost(post)}
                  style={{ cursor: 'pointer', background: '#0a1628', border: '1px solid rgba(31,74,117,.5)', padding: '20px 24px', transition: 'border-color .2s', position: 'relative' }}>
      ...
      {/* 모달 */}
      {selectedPost && (
        <div onClick={closeModal} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', zIndex: 100, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '40px 16px', overflowY: 'auto' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#0a1628', border: '1px solid rgba(31,74,117,.5)', borderRadius: 8, width: '100%', maxWidth: 720, padding: 'clamp(24px,4vw,48px)', position: 'relative' }}>
            <button onClick={closeModal} style={{ position: 'absolute', top: 16, right: 16, background: 'none', border: 'none', color: '#5a7a9a', cursor: 'pointer', fontSize: 20 }}>✕</button>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              {getProp(selectedPost, '카테고리') && <span style={{ fontFamily: 'monospace', fontSize: 8, padding: '1px 6px', background: '#0e1e35', color: '#5a7a9a' }}>{getProp(selectedPost, '카테고리')}</span>}
              <span style={{ fontFamily: 'monospace', fontSize: 10, color: '#5a7a9a' }}>{getProp(selectedPost, '게시일')}</span>
            </div>
            <h1 style={{ fontSize: 'clamp(20px,3vw,28px)', fontWeight: 700, lineHeight: 1.3, marginBottom: 32, paddingBottom: 24, borderBottom: '1px solid rgba(31,74,117,.5)' }}>
              {getProp(selectedPost, '제목')}
            </h1>
            {loadingBlocks ? (
